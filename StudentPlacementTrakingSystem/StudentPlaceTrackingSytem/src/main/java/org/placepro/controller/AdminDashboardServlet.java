...
public class AdminDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        try (Connection con = DBConnection.getConnection()) {
            ...
            out.println("<title>Admin Dashboard - PlacePro</title>");
            ...
            out.println("<div class='sidebar'>"
                ...
                + "<a class='active' href='admindashboard'><i class='bi bi-speedometer2'></i> Dashboard</a>"
            ...
            out.println("<div class='top-header'>"
                + "<input type='text' class='search-box' placeholder='Search Students / Company'>"
                + "<div><i class='bi bi-bell fs-5'></i><i class='bi bi-gear fs-5 ms-3'></i></div>"
                + "</div>");

            // Main
            out.println("<div class='main'>"
                + "<div class='hero-banner'><h2>Welcome Admin</h2><p>Track. Analyze. Manage. Grow.</p></div>"
                + "<div class='row g-3 mb-4'>"
                + "<div class='col-md-3'><div class='card-box'><h5>Total Students</h5><div class='fs-3 fw-bold'>" + totalStudents + "</div></div></div>"
                + "<div class='col-md-3'><div class='card-box'><h5>Companies</h5><div class='fs-3 fw-bold'>" + totalCompanies + "</div></div></div>"
                + "<div class='col-md-3'><div class='card-box'><h5>Applications</h5><div class='fs-3 fw-bold'>" + totalApplications + "</div></div></div>"
                + "<div class='col-md-3'><div class='card-box'><h5>Selected</h5><div class='fs-3 fw-bold'>" + totalSelected + "</div></div></div>"
                + "</div>");

            // ✅ Placement Applications Table via DAO
            PlacementDao dao = new PlacementDaoImpl();
            ...
            for (Placement p : placements) {
                ...
                out.println("<td>"
                    + "<a class='btn btn-success btn-sm' href='updateStatus?id=" + p.getId() + "&status=Selected'>Select</a> "
                    + "<a class='btn btn-danger btn-sm' href='updateStatus?id=" + p.getId() + "&status=Rejected'>Reject</a>"
                    + "</td>");
                out.println("</tr>");
            }
